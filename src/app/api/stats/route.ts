import { NextResponse } from 'next/server';
import { adminDb } from '@/lib/firebase-admin';

export const dynamic = 'force-dynamic';

export async function GET() {
    try {
        // Read from pre-aggregated stats document
        const statsDoc = await adminDb.collection('aggregated_stats').doc('global').get();
        
        if (!statsDoc.exists) {
            // Fallback: return zeros if aggregated stats not yet created
            // Stats will be populated on first cron run
            console.warn('Aggregated stats document not found. Run /api/cron/update-stats to initialize.');
            return NextResponse.json({
                totalDistricts: 0,
                totalTrees: 0,
                totalOxygen: 0,
                previousTotalTrees: 0,
                previousTotalOxygen: 0,
            });
        }

        const data = statsDoc.data();

        // Fetch previous week's stats for trend comparison
        const previousStatsDoc = await adminDb.collection('aggregated_stats').doc('global_previous_week').get();
        const previousData = previousStatsDoc.exists ? previousStatsDoc.data() : null;

        return NextResponse.json({
            totalDistricts: data?.totalDistricts || 0,
            totalTrees: data?.totalTrees || 0,
            totalOxygen: data?.totalOxygen || 0,
            previousTotalTrees: previousData?.totalTrees || 0,
            previousTotalOxygen: previousData?.totalOxygen || 0,
        });
    } catch (error) {
        console.error('Error fetching stats:', error);
        return NextResponse.json(
            { 
                totalDistricts: 0, 
                totalTrees: 0, 
                totalOxygen: 0,
                previousTotalTrees: 0,
                previousTotalOxygen: 0,
            },
            { status: 500 }
        );
    }
}
