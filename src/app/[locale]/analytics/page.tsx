...
export default function AnalyticsPage() {
  ...
  const filteredRegionalData =
    data?.regionalData.filter(
      (r) => selectedRegion === "all" || r.regionName === selectedRegion,
    ) || [];

  const filteredTimeSeries =
    data?.timeSeries.filter((s) => {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      "download",
      `vayura-analytics-${new Date().toISOString().split("T")[0]}.csv`,
    );
          ...
          <header className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4 print:hidden">
            <div>
              ...
              <p className="text-gray-600">
                Comprehensive insights into oxygen intelligence and
                reforestation
              </p>
            </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                  ...
                  <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <select
                        className="text-sm border-none bg-gray-100 rounded-md px-2 py-1 focus:outline-none focus:ring-2 focus:ring-green-500"
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                  ...
                  <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm lg:col-span-2">
                    ...
                    <div className="h-80 w-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <Treemap
                          data={filteredRegionalData.map((r) => ({
                            name: r.regionName,
                            size: r.metrics.totalTrees,
                          }))}
                          dataKey="size"
                          aspectRatio={4 / 3}
                          stroke="#fff"
                    ...
                    <div className="space-y-6">
                      <div className="p-4 bg-green-50 rounded-lg border border-green-100">
                        ...
                        <p className="text-2xl font-bold text-green-900">
                          {(data.predictive.projectedOxygen2030 / 1000).toFixed(
                            1,
                          )}
                          t
                        </p>
                      ...
                      <div className="p-4 bg-blue-50 rounded-lg border border-blue-100">
                        <p className="text-sm text-blue-700 font-medium mb-1">
                          Trees to Self-Sufficiency
                        </p>
                        <p className="text-2xl font-bold text-blue-900">
                          {data.predictive.treesNeededForSelfSufficiency.toLocaleString()}
                        </p>
                        <p className="text-xs text-blue-600 mt-1">
                          Target for 100% O2 coverage
                        </p>
                      </div>
                      <div className="p-4 bg-purple-50 rounded-lg border border-purple-100">
                        ...
                        <p className="text-2xl font-bold text-purple-900">
                          {data.predictive.currentGrowthRate.toFixed(0)}{" "}
                          trees/mo
                        </p>
                        <p className="text-xs text-purple-600 mt-1">
                          Based on last 6 months activity
                        </p>
                      </div>
                    </div>
                        ...
                        {data.regionalData.slice(0, 10).map((region, idx) => (
                          ...
                          >
                            ...
                            <td className="px-6 py-4 text-gray-600">
                              {region.metrics.totalTrees.toLocaleString()}
                            </td>
                            <td className="px-6 py-4 text-gray-600">
                              {region.metrics.oxygenProduction.toLocaleString()}
                            </td>
                            <td className="px-6 py-4 text-gray-600">
                              {region.metrics.avgAQI.toFixed(1)}
                            </td>
                            <td className="px-6 py-4 text-gray-600">
                              {region.metrics.oxygenGap.toLocaleString()}
                            </td>
  ...
  return (
    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        ...
        {trend && (
          <div
            className={`flex items-center text-sm font-medium ${trendUp ? "text-green-600" : "text-red-600"}`}
          >
            {trendUp ? (
